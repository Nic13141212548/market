...
sub getsql
{
    ...
    #########################################################################################
$SQL_BUFF[0]=qq(
set mapreduce.job.name=dwb_bt_rfm_model_s_d_00;
use dwb;
create table if not exists dwb.dwb_bt_rfm_model_s_d
(
 etl_dt                  string                  
,user_pin                string                  
...
use dwb;
insert overwrite table dwb.dwb_bt_rfm_model_s_d partition (dt = '$TX_DATE')
select
